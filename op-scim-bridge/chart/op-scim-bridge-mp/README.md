...
3. Update the `appVersion` of the [Chart.yaml](./Chart.yaml) to match that of the bundled version of the SCIM bridge, i.e. if this release includes SCIM bridge version is "2.5.0" (build 205001), then you would update the `appVersion` to `"2.5.0"`. Note that there's no "v" preceding the `appVersion` value.
