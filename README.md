...
1. Confirm that the `op-scim-bridge` version of the [Chart.yaml](./op-scim-bridge/chart/op-scim-bridge-mp/Chart.yaml) is set to a range, i.e. `~2` instead of a hardcoded version such as `2.5.0`.
2. Update the chart dependencies:
    1. Command: `helm dependency update op-scim-bridge/chart/op-scim-bridge-mp`
    2. This should update the dependencies contained under `op-scim-bridge/chart/op-scim-bridge-mp/charts`.
    3. It will also update the `Chart.lock` file. This file is automatically generated by Helm.
3. Update the `appVersion` of the [Chart.yaml](./op-scim-bridge/chart/op-scim-bridge-mp/Chart.yaml) to match that of the bundled version of the SCIM bridge, i.e. if the SCIM bridge dependency is `op-scim-bridge-2.6.0.tgz`, then you would update the `appVersion` to `v2.6.0`.
